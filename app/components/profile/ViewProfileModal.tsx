...
export default function ViewProfileModal({ profile, onClose }: Props) {
  ...
  return (
    <div className="overlay" onClick={onClose}>
      <div className="viewModal" onClick={(e) => e.stopPropagation()}>
        {/* ── Avatar ── */}
        <img
          src={avatarSrc}
        ...
        {/* ── Name & Role ── */}
        <h2 className="viewName">{profile.display_name || "Cook"}</h2>
        <p className="viewRole">
          {profile.role === "admin" ? "Admin" : "Home Cook"}
        </p>

        {/* ── Bio ── */}
        {profile.bio && <p className="viewBio">{profile.bio}</p>}

        <hr className="viewDivider" />

        {/* ── Email ── */}
        <div className="viewRow">
          <span className="viewIconCircle">
        ...
        <button className="closeOnlyBtn" onClick={onClose}>
          Close
        </button>
      </div>
    </div>
